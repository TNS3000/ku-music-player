...
| Zone 5「House Flow」 | **370** | +270超過 | 118 | 目標達成・運用中 / Target reached |

**合計 / Total: 758 / 500曲（152%）**

> **注:** ローカルには 173〜372（392ファイル）が揃っているが、配信容量の都合で **173〜272（198ファイル）のみを反映**。273〜372（194ファイル / 786MB）は未コミット。詳細は「既知の問題」参照。
...
- **Zone5 087〜172 の番号は欠番**: 001-086（172ファイル）の次が173から。旧「172曲」はファイル数であり連番の上限ではなかった。採番時に注意。
- **Zone5 A/B片側欠損**: `KU-Z5-263`（Bのみ）・`KU-Z5-264`（Aのみ）。SUNOで対になる側の再生成が必要。
- **Zone5 341/342/367 欠番**: 173〜372の連番のうちこの3番号が生成されていない。
- **配信容量の問題（重要）**: GitHub Pages の公開サイト上限は1GB。今回の反映後で約2.47GB（zone1-4=1,043MB + zone5=1,433MB）と大幅超過。`.git` も約4GB。**273〜372（194ファイル / 786MB）は未コミットで保留中**。恒久対応として音源をCloudflare R2等の外部ストレージに移し、index.html からURL参照する構成への移行を検討すること。
- **Zone5 173番以降のサフィックスが小文字**: `-a` / `-b`（086以前は `-A` / `-B`）。index.html は実ファイル名どおり小文字で登録済み（GitHub Pagesは大小文字を区別するため）。将来まとめて大文字へリネームする場合は index.html も同時更新すること。
...
- Zone 5: Repo=HTML=370 ✓ / ローカル作業ディレクトリのみ564（273〜372の194ファイルは未コミット）。HDDはmacOS権限の都合で未照合。2026-07-22時点
...
### 2026-07-22 の作業
- **Zone5追加（部分反映）**: HDDから173〜372（392ファイル）をローカルにコピー。うち **173〜272（198ファイル / 792MB）のみをコミット**し index.html に登録。Zone5配信370ファイル、総計758曲。
- **273〜372 を保留**: 配信サイズが上限を大きく超えるため未コミット（下記「配信容量の問題」参照）。ファイルは `zone5/` に未追跡のまま残置。
- **整合性検証**: index.htmlの370エントリが全てディスク上の実ファイルに解決（リンク切れ0・未登録0・重複0）。JS構文チェック済み。
- **既知の問題を追記**: Zone5の欠番（087-172・341・342・367）、A/B片側欠損（263・264）、173番以降のサフィックス小文字化。
...
5. **Zone5 263-A / 264-B を生成**: 片側欠損の解消（118bpm / Zone5スタイル）
6. **配信構成の見直し（最優先級）**: 音源を外部ストレージ（Cloudflare R2等）へ移行。完了後に保留中の 273〜372 を反映する
...
- Zone5：配信370ファイル / ローカル564ファイル（2026-07-22時点、連番は001-086と173-372）。次バッチは KU-Z5-373 から採番。
